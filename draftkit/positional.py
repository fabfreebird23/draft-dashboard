...
These numbers are ESPN's own, not an inference: `positionLimits` on the league
reads {QB 2, RB 4, WR 4, TE 2, K 2, D/ST 2} in both 2025 and 2026, and they sum
to 16 against a roster of 17 (9 starters + 8 bench, IR excluded). THAT is the
"extra slot we can't use": the eighth bench spot cannot legally be filled by
anybody, because every position is already at its maximum. It is a quirk of the
league's settings, not something we can draft into.

The 17th round ESPN reports is a phantom — twelve picks with playerId -1 and
nobody in them. The draft is 16 real rounds.

(Worth knowing, though it does not change the shape: in the 2025 draft every one
of the twelve teams entered exactly FOURTEEN picks, not sixteen. Uniform across
the league, so it is unlikely to be sloppy data entry on one team's part — the
last two rounds may simply not get typed in, or may not be drafted at all.)
"""
