...
def _pick_cards(ctx, sugg, *, key_prefix, pick_no, next_pick, queued, on_star,
    ...
    """The Suggestions list as pick STRIPS — the card at a tenth of the height.

    The full card was 277px, so the war room's middle column showed one and a half
    of them. On the clock the job is comparison, and a screen showing one player
    cannot do it. Everything still comes from the same `top_suggestions` result the
    rows use; the difference is what gets a line of its own and what gets folded
    into the subtitle.
    """
    from .. import value as V
    reg = ctx["registry"]
    ...
    for s in sugg:
        ...
        # ONE reason, and the tone of the strip comes from it. The full card listed
        # three; three is a card you read, one is a card you glance at.
        reason, tone = "", ""
        if s.get("bye_clash"):
            reason, tone = (f'<i class="bad">bye {byes.get(pm.team, "")} clashes</i>', "bad")
        elif mult >= 0.999:
            reason, tone = (f'<i class="go">fills {pos} starter</i>', "go")
        elif sv is not None and sv < 45:
            reason, tone = (f'<i class="warn">~{100 - sv:.0f}% gone by '
                            f'{_pick_label(next_pick) or "your turn"}</i>', "warn")
        elif left is not None and 1 <= left <= 3:
            reason, tone = (f'<i class="warn">{left} startable {pos} left</i>', "warn")
        elif s.get("stack"):
            reason, tone = ('<i class="go">stacks with your QB</i>', "go")
        elif v is not None:
            reason = f'<i>best value left</i>'

        bits = [f'{pos} · {pm.team}',
                f'<i>{ctx["pos_rank"].get(pid, pos)}</i>',
                f'ADP {int(adp)}' if adp else "ADP —"]
        # ...but not "bye 6 · bye 6 clashes". When the bye IS the reason it is
        # already being said, and a strip has no room to say anything twice.
        if byes.get(pm.team) and not s.get("bye_clash"):
            bits.append(f'bye {byes[pm.team]}')
        if reason:
            bits.append(reason)

        cols = st.columns([1.0, 7.2, 0.5], gap="small")
        if quick_draft:
            with cols[0], st.container(key=f"{key_prefix}_pcd2_{pid}"):
                if st.button("Draft", key=f"{key_prefix}_pcd_{pid}",
                             use_container_width=True):
                    quick_draft(pid)
        with cols[1]:
            st.markdown(C.pick_strip_html(
                name=r.get("name") or pm.name, pid=pid,
                sub=" · ".join(bits),
                wash=theme.team_wash(pm.team, "transparent"),
                value=v, survival=sv, tone=tone), unsafe_allow_html=True)
        with cols[2], st.container(key=f"{key_prefix}_pcq2_{pid}"):
            starred = pid in queued_s
            if st.button("★" if starred else "☆", key=f"{key_prefix}_pcq_{pid}") and on_star:
                on_star(pid)
